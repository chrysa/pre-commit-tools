...
class TestDeadCodeDetectionMain:
    def test_missing_vulture_returns_1(self, mocker: MockerFixture) -> None:
        mocker.patch.dict('sys.modules', {'vulture': None})
        assert main([]) == 1
    ...
    def test_unused_function_returns_1(self, tmp_path: Path, mocker: MockerFixture) -> None:
        ...
        mocker.patch.object(vlt.Vulture, 'get_unused_code', return_value=[mock_item])
        assert main(['--min-confidence=60', f]) == 1
